<?php
include "../library.php";
$_SESSION["page"]="articles";
@$rech=$_GET["rech"];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.2/css/all.min.css"/>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css" rel="stylesheet" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.3.0/css/all.min.css" integrity="sha512-SzlrxWUlpfuzQ+pcUCosxcglQRNAq/DZjVsC0lE40xsADsfeQoEypE+enwcOiGjk/bSuGGKHEyjSoQ1zVisanQ==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <link rel="stylesheet" href="../styles/admin_style.css">
    <link rel="shortcut icon" href="../images/Shotballshop.png" type="image/x-icon">
    <script src="../JavaScript/admin.js"></script>
    <title>Shotballshop - Administration</title>
</head>
<body>
    <div id="edit_cont">
        <div id="edit">
            <a id="close_edit" onclick="close_cru()">fermer</a>
            <br>
            <form id="crud" method="post">
                <img src="" alt="Image" id="img_art"><br>
                designation:<input type="text" name="name" id="name">
                <br>
                <!-- Prix ttc:<input type="text" name="ttc" id="ttc">
                <br> -->
                Prix ht:<input type="text" name="ht" id="ht">
                <br>
                TVA en (%):<input type="text" name="tva" id="tva">
                <br>
                qte de stock:<input type="text" name="stock" id="stock">
                <br>
                Image:<input type="file" name="img_src" id="img_src" onchange="load_img()">
                <label for="img_src"></label>
                <br>
                Cat&eacute;gorie: 
                <select name="categorie" id="categorie">
                    <?php
                        $equipe=new table("localhost","root","","website","categories");
                        $equipe->load_table("1","option");
                    ?>
                </select>
                <!-- <input type="text" name="prix" id="prix"> -->
                <br>
                Equipe: 
                <select name="equipe" id="equipe">
                    <?php
                        $equipe=new table("localhost","root","","website","equipes");
                        $equipe->load_table("1","option");
                    ?>
                    
                </select>
                <br>
                Marque:<input type="text" name="marque" id="marque">
                <br>
                Couleur:<input type="color" name="color" id="color">
                <br>
                <input type="hidden" name="poids" id="poids" value="0">
                <br>
                <button id="ajouter_btn" onclick="cru_val(0)">ajouter</button>
                <button id="modifier_btn" onclick="cru_val(1)">modifier</button>
                <button id="suppr_btn" onclick="cru_val(2)">supprimer</button>
                <input type="hidden" name="operation" id="index">
                <input type="hidden" name="nrech" id="nrech">
                <input type="hidden" name="admin" id="admin">
            </form>
        </div>
    </div>
    <?php
        include "header.php";
    ?>
    <div id="container">
        <center>
            <p id="title">
                Liste des articles
            </p>
        </center>
        <header>
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="get">
                <input type="text" name="rech" id="rech" value="<?php if(isset($rech)) echo $rech; ?>" onkeyup="search(this.value)">
                <input type="submit" value="rechercher">
                <span id="nbr_art">article(s)</span>
                <span id="nbr_selec"></span>
                <a id="ajouter" onclick="show_cru()">Ajouter</a>
            </form>
        </header>
        <section>
            <form method="get" action="delete.php" id="minicontainer">
                <table class='class1'>
                    <thead>
                        <tr class="entete">
                            <td><a href="">Designation</a></td>
                            <td><a href="">Prix ttc</a></td>
                            <td><a href="">Prix ht</a></td>
                            <td><a href="">TVA</a></td>
                            <td><a href="">Quantit&eacute; en stock</a></td>
                            <td><a href="">Image</a></td>
                            <td><a href="">Cat&eacute;gorie</a></td>
                            <td><a href="">Equipe</a></td>
                            <td><a href="">Marque</a></td>
                            <td><a href="">Couleur</a></td>
                            <!-- <td><a href="">Poids</a></td> -->
                            <td><a href="">Date d'ajout</a></td>
                            <td><a href="">Nombre de recherches</a></td>
                            <td><a href="">Cr&eacute;e par</a></td>
                            <td></td>
                            <td></td>
                        </tr>
                    </thead>
                    <tbody id="class1">

                    </tbody>
                </table>
                <input type="hidden" name="selected" id="selected">
            </form>
        </section>
    </div>
</body>
</html>